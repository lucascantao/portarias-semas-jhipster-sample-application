...
import { EntityArrayResponseType, AssuntoService } from '../service/assunto.service';
...
export class AssuntoComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAssunto): number => this.assuntoService.getAssuntoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.assuntos = this.refineData(dataFromBody);
  }

  protected refineData(data: IAssunto[]): IAssunto[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IAssunto[] | null): IAssunto[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.assuntoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
